...
static uint8_t ad_stream_read(struct ad_stream *stream, uint8_t *buf, uint8_t buf_len)
{
    ...
    while (read_len < buf_len)
    {
        ...
        else
        {
            ...
            (void)memcpy(&buf[read_len], &current_ltv->data[stream->current_ltv_offset - 2],
                         size_to_copy);
...
enum adv_name_type get_adv_name_type_param(const struct bt_le_adv_param *param)
{
    if (param->options & BT_LE_ADV_OPT_USE_NAME)
    {
        ...
        if ((param->options & BT_LE_ADV_OPT_EXT_ADV) && !(param->options & BT_LE_ADV_OPT_SCANNABLE))
        {
            return ADV_NAME_TYPE_AD;
        }
...
{
    if (handle < ARRAY_SIZE(adv_pool) && atomic_test_bit(adv_pool[handle].flags, BT_ADV_CREATED))
    {
...
static bool valid_adv_ext_param(const struct bt_le_adv_param *param)
{
    if (IS_ENABLED(CONFIG_BT_EXT_ADV) && BT_DEV_FEAT_LE_EXT_ADV(bt_dev.le.features))
    {
        ...
        if (!(param->options & BT_LE_ADV_OPT_EXT_ADV) &&
            param->options & (BT_LE_ADV_OPT_EXT_ADV | BT_LE_ADV_OPT_NO_2M | BT_LE_ADV_OPT_CODED |
...
static bool ad_is_limited(const struct bt_data *ad, size_t ad_len)
{
    ...
    for (i = 0; i < ad_len; i++)
    {
        if (ad[i].type == BT_DATA_FLAGS && ad[i].data_len == sizeof(uint8_t) && ad[i].data != NULL)
        {
...
static uint8_t get_filter_policy(uint32_t options)
{
    ...
    else if ((options & BT_LE_ADV_OPT_FILTER_SCAN_REQ) && (options & BT_LE_ADV_OPT_FILTER_CONN))
    {
...
static void le_adv_stop_free_conn(const struct bt_le_ext_adv *adv, uint8_t status)
{
    ...
    if (!bt_addr_le_cmp(&adv->target_addr, BT_ADDR_LE_ANY))
    {
        conn = bt_conn_lookup_state_le(adv->id, BT_ADDR_LE_NONE, BT_CONN_CONNECTING_ADV);
    }
    else
    {
        conn = bt_conn_lookup_state_le(adv->id, &adv->target_addr, BT_CONN_CONNECTING_DIR_ADV);
    }
                           ...
                           size_t sd_len)
{
    ...
    else if ((param->options & BT_LE_ADV_OPT_SCANNABLE) || sd || (name_type == ADV_NAME_TYPE_SD))
    {
        scannable = true;
    ...
    if (IS_ENABLED(CONFIG_BT_PERIPHERAL) && (param->options & BT_LE_ADV_OPT_CONNECTABLE))
    {
        err = le_adv_start_add_conn(adv, &conn);
        if (err)
        {
            if (err == -ENOMEM && !dir_adv && !(param->options & BT_LE_ADV_OPT_ONE_TIME))
            {
                goto set_adv_state;
            }
...
set_adv_state:
    atomic_set_bit_to(adv->flags, BT_ADV_PERSIST,
                      !dir_adv && !(param->options & BT_LE_ADV_OPT_ONE_TIME));
    ...
    atomic_set_bit_to(adv->flags, BT_ADV_CONNECTABLE, param->options & BT_LE_ADV_OPT_CONNECTABLE);

    atomic_set_bit_to(adv->flags, BT_ADV_SCANNABLE, scannable);

    atomic_set_bit_to(adv->flags, BT_ADV_USE_IDENTITY, param->options & BT_LE_ADV_OPT_USE_IDENTITY);
...
void bt_le_adv_resume(void)
{
    ...
    if (err)
    {
        BT_DBG("Controller cannot resume connectable advertising (%d)", err);
        bt_conn_set_state(conn, BT_CONN_DISCONNECTED);
...
int bt_le_ext_adv_update_param(struct bt_le_ext_adv *adv, const struct bt_le_adv_param *param)
{
    ...
    if (IS_ENABLED(CONFIG_BT_PER_ADV) && atomic_test_bit(adv->flags, BT_PER_ADV_PARAMS_SET))
    {
         ...
         */
        if (param->options & BT_LE_ADV_OPT_CONNECTABLE ||
            param->options & BT_LE_ADV_OPT_SCANNABLE || !(param->options & BT_LE_ADV_OPT_EXT_ADV) ||
                           ...
                           const struct bt_data *sd, size_t sd_len)
{
    ...
    return le_adv_update(adv, ad, ad_len, sd, sd_len, ext_adv, scannable, get_adv_name_type(adv));
}
...
void bt_hci_le_adv_set_terminated(struct net_buf *buf)
{
    ...
    BT_DBG("status 0x%02x adv_handle %u conn_handle 0x%02x num %u", evt->status, evt->adv_handle,
           conn_handle, evt->num_completed_ext_adv_evts);
    ...
    if (IS_ENABLED(CONFIG_BT_CONN) && !evt->status)
    {
        ...
        if (conn)
        {
            if (IS_ENABLED(CONFIG_BT_PRIVACY) && !atomic_test_bit(adv->flags, BT_ADV_USE_IDENTITY))
            {
                ...
                if (bt_addr_cmp(&conn->le.resp_addr.a, BT_ADDR_ANY) == 0)
                {
...
void bt_hci_le_scan_req_received(struct net_buf *buf)
{
    ...
    if (adv->cb && adv->cb->scanned)
    {
        ...
        if (evt->addr.type == BT_ADDR_LE_PUBLIC_ID || evt->addr.type == BT_ADDR_LE_RANDOM_ID)
        {
