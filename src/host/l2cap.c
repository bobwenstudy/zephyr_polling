...
struct bt_l2cap_fixed_chan
        *l2cap_fixed_chan[CONFIG_BT_L2CAP_FIXED_CHAN_SIZE] __noretention_data_section;
...
static struct net_buf *l2cap_create_le_sig_pdu(struct net_buf *buf, uint8_t code, uint8_t ident,
                                               uint16_t len)
{
...
static int l2cap_le_conn_req(struct bt_l2cap_le_chan *ch)
{
    ...
    buf = l2cap_create_le_sig_pdu(NULL, BT_L2CAP_LE_CONN_REQ, ch->ident, sizeof(*req));
    if (!buf)
    {
        return -ENOMEM;
    }
...
static int l2cap_ecred_conn_req(struct bt_l2cap_chan **chan, int channels)
{
    ...
    for (i = 0; i < channels; i++)
    {
        ...
        __ASSERT(ch->psm == req_psm, "The PSM shall be the same for channels in the same request.");
...
struct net_buf *bt_l2cap_create_pdu_timeout(struct spool *pool, size_t reserve, k_timeout_t timeout)
{
    return bt_conn_create_pdu_timeout(pool, sizeof(struct bt_l2cap_hdr) + reserve, timeout);
}

int bt_l2cap_send_cb(struct bt_conn *conn, uint16_t cid, struct net_buf *buf, bt_conn_tx_cb_t cb,
                     void *user_data)
{
...
static void le_conn_param_update_req(struct bt_l2cap *l2cap, uint8_t ident, struct net_buf *buf)
{
    ...
    BT_DBG("min 0x%04x max 0x%04x latency: 0x%04x timeout: 0x%04x", param.interval_min,
...
static void l2cap_chan_rx_init(struct bt_l2cap_le_chan *chan)
{
    ...
    if (!chan->rx.init_credits)
    {
        if (chan->chan.ops->alloc_buf)
        {
            ...
            chan->rx.init_credits = ceiling_fraction(chan->rx.mtu, BT_L2CAP_RX_MTU);
        }
        else
        {
    ...
    if (BT_DBG_ENABLED &&
        chan->rx.init_credits * chan->rx.mps < chan->rx.mtu + BT_L2CAP_SDU_HDR_SIZE)
    {
...
static int l2cap_chan_le_send_sdu(struct bt_l2cap_le_chan *ch, struct net_buf **buf, uint16_t sent);
...
static void l2cap_chan_rx_give_credits(struct bt_l2cap_le_chan *chan, uint16_t credits)
{
    BT_DBG("chan %p credits %u", chan, credits);
...
static void le_ecred_conn_req(struct bt_l2cap *l2cap, uint8_t ident, struct net_buf *buf)
{
    ...
    while (buf->len >= sizeof(scid))
    {
        ...
        case BT_L2CAP_LE_SUCCESS:
            ...
            continue;
        /* Some connections refused �C invalid Source CID */
        /* Some connections refused �C Source CID already allocated */
        /* Some connections refused �C not enough resources
         * available.
         */
        default:
...
static void le_ecred_reconf_req(struct bt_l2cap *l2cap, uint8_t ident, struct net_buf *buf)
{
    ...
    while (buf->len >= sizeof(scid))
    {
        ...
        {
            BT_ERR("chan %p decreased MTU %u -> %u", chan, BT_L2CAP_LE_CHAN(chan)->tx.mtu, mtu);
            result = BT_L2CAP_RECONF_INVALID_MTU;
            goto response;
        }
...
static void le_ecred_conn_rsp(struct bt_l2cap *l2cap, uint8_t ident, struct net_buf *buf)
{
    ...
    BT_DBG("mtu 0x%04x mps 0x%04x credits 0x%04x result %u", mtu, mps, credits, result);
    ...
    /* Some connections refused �C not enough resources available */
...
static struct net_buf *l2cap_chan_create_seg(struct bt_l2cap_le_chan *ch, struct net_buf *buf,
                                             size_t sdu_hdr_len)
{
    struct net_buf *seg;
    uint16_t headroom;
...
{
    if (!atomic_get(&ch->tx.credits) || (k_fifo_is_empty(&ch->tx_queue) && !ch->tx_buf))
    {
        return;
    }
                              ...
                              uint16_t sdu_hdr_len)
{
    ...
    BT_DBG("ch %p cid 0x%04x len %u credits %lu", ch, ch->tx.cid, seg->len,
    ...
    if (buf == seg || !buf->len)
    {
        err = bt_l2cap_send_cb(ch->chan.conn, ch->tx.cid, seg, l2cap_chan_sdu_sent,
                               l2cap_tx_meta_data(buf));
    }
    else
    {
        err = bt_l2cap_send_cb(ch->chan.conn, ch->tx.cid, seg, l2cap_chan_seg_sent,
                               l2cap_tx_meta_data(buf));
    }
...
static void le_credits(struct bt_l2cap *l2cap, uint8_t ident, struct net_buf *buf)
{
    ...
    BT_DBG("chan %p total credits %lu", le_chan, atomic_get(&le_chan->tx.credits));
...
static void l2cap_chan_send_credits(struct bt_l2cap_le_chan *chan, struct net_buf *buf,
                                    uint16_t credits)
{
...
static void l2cap_chan_le_recv_seg(struct bt_l2cap_le_chan *chan, struct net_buf *buf)
{
    ...
    if (net_buf_frags_len(chan->_sdu) < chan->_sdu_len)
    {
         ...
         * MPS for some reason.
         */
        if (!atomic_get(&chan->rx.credits) && seg == chan->rx.init_credits)
        {
...
{
    BT_DBG("ch %p cid 0x%04x", BT_L2CAP_LE_CHAN(chan), BT_L2CAP_LE_CHAN(chan)->rx.cid);
}
...
BT_L2CAP_CHANNEL_DEFINE_NEW(le_fixed_chan, BT_L2CAP_CID_LE_SIG, l2cap_accept, NULL);
...
static int l2cap_ecred_init(struct bt_conn *conn, struct bt_l2cap_le_chan *ch, uint16_t psm)
{
    ...
    BT_DBG("ch %p psm 0x%02x mtu %u mps %u credits %u", ch, ch->psm, ch->rx.mtu, ch->rx.mps,
           ch->rx.init_credits);
...
int bt_l2cap_ecred_chan_reconfigure(struct bt_l2cap_chan **chans, uint16_t mtu)
{
    ...
    buf = l2cap_create_le_sig_pdu(NULL, BT_L2CAP_ECRED_RECONF_REQ, ident,
...
int bt_l2cap_chan_disconnect(struct bt_l2cap_chan *chan)
{
    ...
    BT_DBG("chan %p scid 0x%04x dcid 0x%04x", chan, le_chan->rx.cid, le_chan->tx.cid);

    le_chan->ident = get_ident();

    buf = l2cap_create_le_sig_pdu(NULL, BT_L2CAP_DISCONN_REQ, le_chan->ident, sizeof(*req));
    if (!buf)
    {
        return -ENOMEM;
    }
