...
void storage_kv_set_id(void)
{
    ...
    storage_kv_set(KEY_INDEX_LE_ID_IRK_LIST, bt_dev.irk, ID_DATA_LEN(bt_dev.irk));
#endif
}
...
static void adv_is_limited_enabled(struct bt_le_ext_adv *adv, void *data)
{
    ...
    if (atomic_test_bit(adv->flags, BT_ADV_ENABLED) && atomic_test_bit(adv->flags, BT_ADV_LIMITED))
    {
        *adv_enabled = true;
    }
}
...
{
    /* Invalidate RPA */
    if (!(IS_ENABLED(CONFIG_BT_EXT_ADV) && atomic_test_bit(bt_dev.flags, BT_DEV_SCAN_LIMITED)))
    {
...
static void adv_enable_rpa(struct bt_le_ext_adv *adv, void *data)
{
    ...
    if (adv_disabled[adv_index])
    {
        ...
        if (err)
        {
            BT_WARN("Failed to update advertiser RPA address (%d)", err);
        }
...
static void le_update_private_addr(void)
{
    ...
    if (atomic_test_bit(bt_dev.flags, BT_DEV_SCANNING) &&
        !(IS_ENABLED(CONFIG_BT_EXT_ADV) && atomic_test_bit(bt_dev.flags, BT_DEV_SCAN_LIMITED)))
    {
        bt_le_scan_set_enable(BT_HCI_LE_SCAN_DISABLE);
        scan_enabled = true;
    }
#endif
    if (IS_ENABLED(CONFIG_BT_CENTRAL) && IS_ENABLED(CONFIG_BT_FILTER_ACCEPT_LIST) &&
...
static void rpa_timeout(struct k_work *work)
{
    ...
    {
        struct bt_conn *conn =
                bt_conn_lookup_state_le(BT_ID_DEFAULT, NULL, BT_CONN_CONNECTING_SCAN);
...
bool bt_id_adv_random_addr_check(const struct bt_le_adv_param *param)
{
             ...
             bt_dev.id_addr[BT_ID_DEFAULT].type == BT_ADDR_LE_RANDOM)
    {
        ...
        else if (bt_dev.id_addr[param->id].type == BT_ADDR_LE_RANDOM && param->id != BT_ID_DEFAULT)
        {
...
{
    BT_DBG("id: %d, type: %d, addr: %s, irk: %p", id, addr->type, bt_addr_str_real(&addr->a), irk);
        ...
        *own_addr_type = bt_dev.id_addr[0].type;
        ...
        if (!IS_ENABLED(CONFIG_BT_SCAN_WITH_IDENTITY) && !is_adv_using_rand_addr())
        {
            ...
            if (err)
            {
                ...
                BT_WARN("Ignoring failure to set address for passive scan (%d)", err);
            }

            *own_addr_type = BT_ADDR_LE_RANDOM;
        }
        else if (IS_ENABLED(CONFIG_BT_SCAN_WITH_IDENTITY) && *own_addr_type == BT_ADDR_LE_RANDOM)
        {
                           ...
                           uint8_t *own_addr_type)
{
    ...
    else
    {
        ...
        else if (!(IS_ENABLED(CONFIG_BT_EXT_ADV) && BT_DEV_FEAT_LE_EXT_ADV(bt_dev.le.features)))
        {
