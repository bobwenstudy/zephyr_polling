...
NET_BUF_POOL_DEFINE(prep_pool, CONFIG_BT_ATT_PREPARE_COUNT, BT_ATT_MTU, sizeof(struct bt_attr_data),
                    NULL);
#endif /* CONFIG_BT_ATT_PREPARE_COUNT */

K_MEM_SLAB_NORETENTION_DEFINE(req_slab, sizeof(struct bt_att_req), CONFIG_BT_L2CAP_TX_BUF_COUNT,
...
static struct bt_att_tx_meta_data tx_meta_data[CONFIG_BT_CONN_TX_MAX] __noretention_data_section;
...
static inline void tx_meta_data_free(struct bt_att_tx_meta_data *data)
{
    ...
    k_fifo_put(&free_att_tx_meta_data, &data->node);
}
...
static int chan_send(struct bt_att_chan *chan, struct net_buf *buf)
{
    ...
    if (IS_ENABLED(CONFIG_BT_EATT) && atomic_test_bit(chan->flags, ATT_ENHANCED))
    {
    ...
    err = bt_l2cap_send_cb(chan->att->conn, BT_L2CAP_CID_ATT, buf, att_cb(buf), data);
    if (err)
    {
...
struct net_buf *bt_att_chan_create_pdu(struct bt_att_chan *chan, uint8_t op, size_t len)
{
    ...
    {
        BT_WARN("ATT MTU exceeded, max %u, wanted %zu", chan->chan.tx.mtu, len + sizeof(op));
        return NULL;
    }
...
static uint8_t att_handle_rsp(struct bt_att_chan *chan, void *pdu, uint16_t len, uint8_t err)
{
    bt_att_func_t func = NULL;
    void *params;

    BT_DBG("chan %p err 0x%02x len %u: %s", chan, err, len, bt_hex(pdu, len));
...
static uint8_t find_info_cb(const struct bt_gatt_attr *attr, uint16_t handle, void *user_data)
{
    ...
    case BT_ATT_INFO_16:
        ...
        if (chan->chan.tx.mtu - data->buf->len > sizeof(*data->info16))
        {
    ...
    case BT_ATT_INFO_128:
        ...
        if (chan->chan.tx.mtu - data->buf->len > sizeof(*data->info128))
        {
            return BT_GATT_ITER_CONTINUE;
        }
    }
...
static uint8_t find_type_cb(const struct bt_gatt_attr *attr, uint16_t handle, void *user_data)
{
    ...
    if (chan->chan.tx.mtu - net_buf_frags_len(data->buf) < sizeof(*data->group))
    {
        return BT_GATT_ITER_STOP;
    }
    ...
    len = MIN(chan->chan.tx.mtu - net_buf_frags_len(data->buf), net_buf_tailroom(frag));
    if (!len)
    {
        frag = net_buf_alloc(data->buf->pool_id, K_NO_WAIT);
...
static uint8_t att_find_type_req(struct bt_att_chan *chan, struct net_buf *buf)
{
    ...
    {
        send_err_rsp(chan, BT_ATT_OP_FIND_TYPE_REQ, start_handle, BT_ATT_ERR_ATTRIBUTE_NOT_FOUND);
        return 0;
    }
                             ...
                             struct net_buf *buf, uint16_t offset, attr_read_cb cb, void *user_data)
{
    ...
    do
    {
        len = MIN(chan->chan.tx.mtu - net_buf_frags_len(buf), net_buf_tailroom(frag));
        if (!len)
        {
            frag = net_buf_alloc(buf->pool_id, K_NO_WAIT);
...
static uint8_t read_type_cb(const struct bt_gatt_attr *attr, uint16_t handle, void *user_data)
{
    ...
    return chan->chan.tx.mtu - net_buf_frags_len(data->buf) > data->rsp->len ? BT_GATT_ITER_CONTINUE
                                                                             : BT_GATT_ITER_STOP;
}
...
static uint8_t att_read_type_req(struct bt_att_chan *chan, struct net_buf *buf)
{
    ...
    BT_DBG("start_handle 0x%04x end_handle 0x%04x type %s", start_handle, end_handle,
           bt_uuid_str(&u.uuid));
...
static uint8_t read_group_cb(const struct bt_gatt_attr *attr, uint16_t handle, void *user_data)
{
    ...
    /* Stop if there is no space left */
    if (data->rsp->len && chan->chan.tx.mtu - data->buf->len < data->rsp->len)
    {
        return BT_GATT_ITER_STOP;
    }
                                  ...
                                  uint16_t start_handle, uint16_t end_handle)
{
    ...
    if (!data.rsp->len)
    {
        ...
        net_buf_unref(data.buf);
        /* Respond here since handle is set */
        send_err_rsp(chan, BT_ATT_OP_READ_GROUP_REQ, start_handle, BT_ATT_ERR_ATTRIBUTE_NOT_FOUND);
        return 0;
    }
                                     ...
                                     struct net_buf_simple *buf)
{
    ...
    SYS_SLIST_FOR_EACH_CONTAINER_SAFE (list, entry, next, node)
    {
        ...
        BT_DBG("entry %p handle 0x%04x, offset %u", entry, tmp_data->handle, tmp_data->offset);
        ...
        if (tmp_data->handle == handle)
        {
            ...
            if (tmp_data->offset != buf->len + offset)
            {
                ...
                BT_DBG("Bad offset %u (%u, %u)", tmp_data->offset, buf->len, offset);
...
static uint8_t att_exec_write_rsp(struct bt_att_chan *chan, uint8_t flags)
{
    ...
    while (!sys_slist_is_empty(&chan->att->prep_queue))
    {
        ...
        NET_BUF_SIMPLE_DEFINE_STATIC(
                reassembled_data,
                MIN(BT_ATT_MAX_ATTRIBUTE_LEN, CONFIG_BT_ATT_PREPARE_COUNT * BT_ATT_MTU));

        buf = net_buf_slist_get(&chan->att->prep_queue);
        data = net_buf_user_data(buf);
        handle = data->handle;

        BT_DBG("buf %p handle 0x%04x offset %u", buf, handle, data->offset);
...
static uint8_t att_error_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
    ...
    BT_DBG("request 0x%02x handle 0x%04x error 0x%02x", rsp->request, sys_le16_to_cpu(rsp->handle),
           rsp->error);
...
static int bt_att_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
    ...
    BT_DBG("Received ATT chan %p code 0x%02x len %zu", att_chan, hdr->code, net_buf_frags_len(buf));
...
static struct bt_att *att_get(struct bt_conn *conn)
{
    ...
    __ASSERT(atomic_test_bit(att_chan->flags, ATT_CONNECTED), "ATT channel not connected");
...
static void att_reset(struct bt_att *att)
{
    ...
    while (!sys_slist_is_empty(&att->reqs))
    {
        ...
        if (req->func)
        {
            req->func(att->conn, BT_ATT_ERR_UNLIKELY, NULL, 0, req->user_data);
        }
...
static void bt_att_encrypt_change(struct bt_l2cap_chan *chan, uint8_t hci_status)
{
    ...
    BT_DBG("chan %p conn %p handle %u sec_level 0x%02x status 0x%02x", le_chan, conn, conn->handle,
           conn->sec_level, hci_status);
...
static void att_enhanced_connection_work_handler(struct k_work *work)
{
    ...
    if (err < 0)
    {
        BT_WARN("Failed to connect %d EATT channels (err: %d)", att->eatt.chans_to_connect, err);
    }
}
#endif /* CONFIG_BT_EATT */
 ...
 */
BT_L2CAP_CHANNEL_DEFINE_NEW(att_fixed_chan, BT_L2CAP_CID_ATT, bt_att_accept, NULL);
...
void bt_att_buf_init(void)
{
    ...
    k_mem_slab_init(&att_slab, att_slab.buffer, att_slab.block_size, att_slab.num_blocks);
    k_mem_slab_init(&chan_slab, chan_slab.buffer, chan_slab.block_size, chan_slab.num_blocks);
    k_mem_slab_init(&req_slab, req_slab.buffer, req_slab.block_size, req_slab.num_blocks);
}
...
{
    if (k_fifo_size(&free_att_tx_meta_data) != ARRAY_SIZE(tx_meta_data))
    {
        return 0;
    }
    if (req_slab.num_used != 0)
    {
        return 0;
    }
...
{
    k_fifo_init(&free_att_tx_meta_data);
    memset(&req_slab, 0, sizeof(struct k_mem_slab));
}
#endif

uint8_t bt_att_get_tx_free_size(void)
{
    return k_fifo_size(&free_att_tx_meta_data);
}
