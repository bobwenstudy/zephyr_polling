...
static struct net_buf *get_first_buf_matching_chan(struct k_fifo *fifo, struct bt_att_chan *chan)
{
    if (IS_ENABLED(CONFIG_BT_EATT))
    {
        ...
        {
            if (!ret && att_chan_matches_chan_opt(chan, bt_att_tx_meta_data(buf)->chan_opt))
            {
                ret = buf;
            }
            else
...
static struct bt_att_req *get_first_req_matching_chan(sys_slist_t *reqs, struct bt_att_chan *chan)
{
    if (IS_ENABLED(CONFIG_BT_EATT))
    {
        ...
        {
            if (att_chan_matches_chan_opt(chan, bt_att_tx_meta_data(ATT_REQ(curr)->buf)->chan_opt))
            {
                break;
            }
...
struct net_buf *bt_att_chan_create_pdu(struct bt_att_chan *chan, uint8_t op, size_t len)
{
    ...
    {
        LOG_WRN("ATT MTU exceeded, max %u, wanted %zu", chan->chan.tx.mtu, len + sizeof(op));
        return NULL;
    }
                                     ...
                                     struct net_buf_simple *buf)
{
    ...
    SYS_SLIST_FOR_EACH_CONTAINER_SAFE (list, entry, next, node)
    {
        ...
        LOG_DBG("entry %p handle 0x%04x, offset %u", entry, tmp_data->handle, tmp_data->offset);
        ...
        if (tmp_data->handle == handle)
        {
            ...
            if (tmp_data->offset != buf->len + offset)
            {
                ...
                LOG_DBG("Bad offset %u (%u, %u)", tmp_data->offset, buf->len, offset);
...
static uint8_t att_error_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
    ...
    LOG_DBG("request 0x%02x handle 0x%04x error 0x%02x", rsp->request, sys_le16_to_cpu(rsp->handle),
            rsp->error);
...
static void cap_eatt_mtu(struct bt_l2cap_le_chan *le_chan)
{
    ...
    le_chan->tx.mps = MIN(le_chan->tx.mps, BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU));
...
static void bt_att_encrypt_change(struct bt_l2cap_chan *chan, uint8_t hci_status)
{
    ...
    LOG_DBG("chan %p conn %p handle %u sec_level 0x%02x status 0x%02x", le_chan, conn, conn->handle,
...
static void att_enhanced_connection_work_handler(struct k_work *work)
{
    ...
    {
        LOG_WRN("Failed to connect %d EATT channels (err: %d)", att->eatt.chans_to_connect, err);
    }
}
#endif /* CONFIG_BT_EATT */
...
bool bt_att_chan_opt_valid(struct bt_conn *conn, enum bt_att_chan_opt chan_opt)
{
    ...
    {
        return (bt_conn_get_security(conn) > BT_SECURITY_L1) && !bt_att_fixed_chan_only(conn);
    }
