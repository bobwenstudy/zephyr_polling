...
struct k_fifo free_tx __noretention_data_section;
...
static void tx_notify(struct bt_conn *conn)
{
    ...
    while (1)
    {
        ...
        {
            tx = CONTAINER_OF(sys_slist_get_not_empty(&conn->tx_complete), struct bt_conn_tx, node);
        }
        // irq_unlock(key);
...
static void bt_acl_recv(struct bt_conn *conn, struct net_buf *buf, uint8_t flags)
{
    ...
    if (conn->rx->len > acl_total_len)
    {
        BT_ERR("ACL len mismatch (%u > %u)", conn->rx->len, acl_total_len);
        bt_conn_reset_rx_state(conn);
        return;
    }
...
static int send_iso(struct bt_conn *conn, struct net_buf *buf, uint8_t flags)
{
    ...
    hdr = net_buf_push(buf, sizeof(*hdr));
    hdr->handle = sys_cpu_to_le16(bt_iso_handle_pack(conn->handle, flags, 0));
    hdr->len = sys_cpu_to_le16(buf->len - sizeof(*hdr));
...
static bool send_frag(struct bt_conn *conn, struct net_buf *buf, uint8_t flags, bool always_consume)
{
...
static int conn_prepare_events(struct bt_conn *conn, struct k_poll_event *events)
{
    ...
    k_poll_event_init(&events[0], K_POLL_TYPE_FIFO_DATA_AVAILABLE, K_POLL_MODE_NOTIFY_ONLY,
                      &conn->tx_queue);
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
    ...
    case BT_CONN_CONNECTING:
        ...
        if (IS_ENABLED(CONFIG_BT_CENTRAL) && conn->type == BT_CONN_TYPE_LE)
        {
            k_work_schedule(&conn->deferred_work, K_MSEC(10 * bt_dev.create_param.timeout));
        }
...
void bt_conn_unref(struct bt_conn *conn)
{
    ...
    BT_DBG("handle %u ref %ld -> %ld", conn->handle, old, atomic_get(&conn->ref));
...
#else
struct net_buf *bt_conn_create_pdu_timeout(struct spool *pool, size_t reserve, k_timeout_t timeout)
#endif
{
    struct net_buf *buf;
...
void notify_le_param_updated(struct bt_conn *conn)
{
    ...
    if (atomic_test_bit(conn->flags, BT_CONN_PERIPHERAL_PARAM_SET) &&
        conn->le.interval >= conn->le.interval_min && conn->le.interval <= conn->le.interval_max &&
...
{
    BT_DBG("conn %p features 0x%02x params (%d-%d %d %d)", conn, conn->le.features[0],
...
bool bt_conn_exists_le(uint8_t id, const bt_addr_le_t *peer)
{
    ...
    if (conn)
    {
         ...
         */
        BT_WARN("Found valid connection in %s state", state2str(conn->state));
        bt_conn_unref(conn);
        return true;
    }
...
#if defined(CONFIG_BT_ISO)
    case BT_CONN_TYPE_ISO:
        if (IS_ENABLED(CONFIG_BT_ISO_UNICAST) && conn->iso.info.type == BT_ISO_CHAN_TYPE_CONNECTED)
        {
...
#if defined(CONFIG_BT_SMP)
    if (IS_ENABLED(CONFIG_BT_PRIVACY) && (bt_dev.le.rl_entries > 0) &&
...
{
    int err, i;
    extern struct bt_l2cap_fixed_chan *l2cap_fixed_chan[];
    ...
    l2cap_fixed_chan[0] = &le_fixed_chan;
    l2cap_fixed_chan[1] = &att_fixed_chan;
    l2cap_fixed_chan[2] = &smp_fixed_chan;
    // l2cap_fixed_chan[3] = &bt_l2cap_fixed_chan;
...
uint8_t bt_conn_check_allow_sleep(void)
{
    int i;
    ...
    for (i = 0; i < ARRAY_SIZE(acl_conns); i++)
    {
        ...
        if (!k_fifo_is_empty(&conn->tx_queue))
        {
            return 0;
        }
        if (!sys_slist_is_empty(&conn->tx_complete))
    ...
    if (k_fifo_size(&free_tx) != ARRAY_SIZE(conn_tx))
    {
        return 0;
    }

    if (!bt_att_check_allow_sleep())
    {
        return 0;
    }
...
void bt_conn_sleep_wake_init(void)
{
    int i;
    ...
    bt_att_sleep_wake_init();
}

#if defined(CONFIG_BT_MONITOR_SLEEP)
void bt_conn_monitor_sleep(void)
{
    k_fifo_init(&free_tx);
    bt_att_monitor_sleep();
}
#endif
