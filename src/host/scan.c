...
static int start_le_scan_ext(struct bt_hci_ext_scan_phy *phy_1m,
                             struct bt_hci_ext_scan_phy *phy_coded, uint16_t duration)
{
...
int bt_le_scan_update(bool fast_scan)
{
    ...
    if (IS_ENABLED(CONFIG_BT_CENTRAL))
    {
        ...
        conn = bt_conn_lookup_state_le(BT_ID_DEFAULT, NULL, BT_CONN_CONNECTING_SCAN);
        if (conn)
        {
                               ...
                               uint8_t adv_props)
{
    ...
    conn = bt_conn_lookup_state_le(BT_ID_DEFAULT, id_addr, BT_CONN_CONNECTING_SCAN);
    if (!conn)
    {
        return;
    }
                        ...
                        struct net_buf_simple *buf, uint16_t len)
{
    ...
    BT_DBG("%s event %u, len %u, rssi %d dBm", bt_addr_le_str(addr), info->adv_type, len,
           info->rssi);
    ...
    if (addr->type == BT_ADDR_LE_PUBLIC_ID || addr->type == BT_ADDR_LE_RANDOM_ID)
    {
...
void bt_hci_le_adv_ext_report(struct net_buf *buf)
{
    ...
    while (num_reports--)
    {
        ...
        is_new_advertiser =
                reassembling_advertiser.state == FRAG_ADV_INACTIVE ||
                !fragmented_advertisers_equal(&reassembling_advertiser, &evt->addr, evt->sid);
...
void bt_hci_le_per_adv_report(struct net_buf *buf)
{
    ...
    if (!per_adv_sync->report_truncated)
    {
        ...
        else
        {
            ...
            if (evt->data_status == BT_HCI_LE_ADV_EVT_TYPE_DATA_STATUS_COMPLETE)
            {
                bt_hci_le_per_adv_report_recv(per_adv_sync, &per_adv_sync->reassembly, &info);
                net_buf_simple_reset(&per_adv_sync->reassembly);
            }
        }
    ...
    if (unexpected_evt || evt->status != BT_HCI_ERR_SUCCESS)
    {
        if (pending_per_adv_sync)
        {
            ...
            SYS_SLIST_FOR_EACH_CONTAINER (&pa_sync_cbs, listener, node)
            {
                if (listener->term)
                {
                    listener->term(pending_per_adv_sync, &term_info);
                }
            }
        }
        return;
...
void bt_hci_le_per_adv_sync_lost(struct net_buf *buf)
{
    ...
    if (!per_adv_sync)
    {
        BT_ERR("Unknown handle 0x%04Xfor periodic adv sync lost", sys_le16_to_cpu(evt->handle));
        return;
    }
...
void bt_hci_le_past_received(struct net_buf *buf)
{
    ...
    sync_info.conn = bt_conn_lookup_handle(sys_le16_to_cpu(evt->conn_handle));
...
int bt_le_scan_start(const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
{
    ...
    if (IS_ENABLED(CONFIG_BT_EXT_ADV) && BT_DEV_FEAT_LE_EXT_ADV(bt_dev.le.features))
    {
        ...
        {
            uint16_t interval = param->interval_coded ? param->interval_coded : param->interval;
            uint16_t window = param->window_coded ? param->window_coded : param->window;
    ...
    else
    {
        ...
        err = start_le_scan_legacy(param->type, param->interval, param->window);
    }
...
uint8_t bt_le_per_adv_sync_get_index(struct bt_le_per_adv_sync *per_adv_sync)
{
    ...
    __ASSERT(index >= 0 && ARRAY_SIZE(per_adv_sync_pool) > index, "Invalid per_adv_sync pointer");
                              ...
                              struct bt_le_per_adv_sync **out_sync)
{
    ...
    if (param->sid > BT_GAP_SID_MAX || param->skip > BT_GAP_PER_ADV_MAX_SKIP ||
        param->timeout > BT_GAP_PER_ADV_MAX_TIMEOUT || param->timeout < BT_GAP_PER_ADV_MIN_TIMEOUT)
    {
        return -EINVAL;
    }
                                          ...
                                          const struct bt_le_per_adv_sync_transfer_param *param)
{
    ...
    if (conn)
    {
        return past_param_set(conn, BT_HCI_LE_PAST_MODE_SYNC, param->skip, param->timeout,
                              cte_type);
    }
    else
    {
        return default_past_param_set(BT_HCI_LE_PAST_MODE_SYNC, param->skip, param->timeout,
                                      cte_type);
    }
}
...
int bt_le_per_adv_sync_transfer_unsubscribe(const struct bt_conn *conn)
{
    ...
    if (conn)
    {
        return past_param_set(conn, BT_HCI_LE_PAST_MODE_NO_SYNC, 0, 0x0a, 0);
    }
    else
    {
        return default_past_param_set(BT_HCI_LE_PAST_MODE_NO_SYNC, 0, 0x0a, 0);
    }
}
#endif /* CONFIG_BT_CONN */
