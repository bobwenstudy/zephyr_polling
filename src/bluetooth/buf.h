...
#ifndef _ZEPHYR_POLLING_BLUETOOTH_BUF_H_
#define _ZEPHYR_POLLING_BLUETOOTH_BUF_H_
...
#define BT_BUF_RX_SIZE (MAX(MAX(BT_BUF_ACL_RX_SIZE, BT_BUF_EVT_RX_SIZE), BT_BUF_ISO_RX_SIZE))

/** Buffer count needed for HCI ACL, HCI ISO or Event RX buffers */
#define BT_BUF_RX_COUNT                                                                            \
    (MAX(MAX(CONFIG_BT_BUF_EVT_RX_COUNT, CONFIG_BT_BUF_ACL_RX_COUNT), BT_BUF_ISO_RX_COUNT))
...
{
    return (enum bt_buf_type)((struct bt_buf_data *)net_buf_user_data(buf))->type;
}
...
#endif /* _ZEPHYR_POLLING_BLUETOOTH_BUF_H_ */