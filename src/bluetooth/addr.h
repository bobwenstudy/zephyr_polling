 ...
 */
#ifndef _ZEPHYR_POLLING_BLUETOOTH_ADDR_H_
#define _ZEPHYR_POLLING_BLUETOOTH_ADDR_H_
...
#define BT_ADDR_NONE    ((bt_addr_t[]){{{0xff, 0xff, 0xff, 0xff, 0xff, 0xff}}})
...
{
    return snprintk(str, len, "%02X:%02X:%02X:%02X:%02X:%02X", addr->val[5], addr->val[4],
...
static inline int bt_addr_le_to_str(const bt_addr_le_t *addr, char *str, size_t len)
{
    ...
    return snprintk(str, len, "%02X:%02X:%02X:%02X:%02X:%02X (%s)", addr->a.val[5], addr->a.val[4],
...
#endif /* _ZEPHYR_POLLING_BLUETOOTH_ADDR_H_ */