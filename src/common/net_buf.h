...
#define NET_BUF_POOL_INITIALIZER(_pool, _alloc, _bufs, _count, _destroy)                           \
    {                                                                                              \
        .free = SYS_SLIST_STATIC_INIT(_pool.free), .buf_count = _count, .uninit_count = _count,    \
        .avail_count = _count,                                                                     \
        .name = STRINGIFY(_pool), .destroy = _destroy, .alloc = _alloc, .__bufs = _bufs,           \
    }
#else
#define NET_BUF_POOL_INITIALIZER(_pool, _alloc, _bufs, _count, _destroy)                           \
    {                                                                                              \
        .free = SYS_SLIST_STATIC_INIT(_pool.free), .buf_count = _count, .uninit_count = _count,    \
        .destroy = _destroy, .alloc = _alloc, .__bufs = _bufs,                                     \
    }
#endif /* CONFIG_NET_BUF_POOL_USAGE */
...
#define NET_BUF_POOL_HEAP_DEFINE(_name, _count, _destroy)                                          \
    ...
    static struct net_buf_pool _name __net_buf_align __in_section(_net_buf_pool, static, _name) =  \
            NET_BUF_POOL_INITIALIZER(_name, &net_buf_heap_alloc, net_buf_##_name, _count,          \
                                     _destroy)
...
#define NET_BUF_POOL_FIXED_DEFINE(_name, _count, _data_size, _destroy)                             \
    ...
    static struct net_buf_pool _name __net_buf_align = NET_BUF_POOL_INITIALIZER(                   \
            _name, &net_buf_fixed_alloc_##_name, net_buf_##_name, _count, _destroy)

#define NET_BUF_POOL_FIXED_DEFINE_PROTECT(_name, _count, _data_size, _destroy)                     \
    static struct net_buf net_buf_##_name[_count];                                                 \
    static uint8_t net_buf_data_##_name[_count][_data_size];                                       \
    static const struct net_buf_pool_fixed net_buf_fixed_##_name = {                               \
            .data_size = _data_size,                                                               \
...
#define NET_BUF_POOL_VAR_DEFINE(_name, _count, _data_size, _destroy)                               \
    ...
    static struct net_buf_pool _name __net_buf_align __in_section(_net_buf_pool, static, _name) =  \
            NET_BUF_POOL_INITIALIZER(_name, &net_buf_data_alloc_##_name, _net_buf_##_name, _count, \
                                     _destroy)
...
struct net_buf *net_buf_alloc_fixed_debug(struct net_buf_pool *pool, k_timeout_t timeout,
...
#define net_buf_alloc(_pool, _timeout) net_buf_alloc_debug(_pool, __func__, __LINE__)
#else
 ...
 */
#if defined(CONFIG_BT_DEBUG)
struct net_buf *net_buf_alloc_len_debug(struct net_buf_pool *pool, size_t size, k_timeout_t timeout,
                                        const char *func, int line);
 ...
 */
#if defined(CONFIG_BT_DEBUG)
struct net_buf *net_buf_alloc_with_data_debug(struct net_buf_pool *pool, void *data, size_t size,
                                              k_timeout_t timeout, const char *func, int line);
#define net_buf_alloc_with_data(_pool, _data_, _size, _timeout)                                    \
    net_buf_alloc_with_data_debug(_pool, _data_, _size, _timeout, __func__, __LINE__)
#else
...
#define net_buf_unref(_buf) net_buf_unref_debug(_buf, __func__, __LINE__)
#else
...
struct net_buf *net_buf_frag_del_debug(struct net_buf *parent, struct net_buf *frag,
                                       const char *func, int line);
#define net_buf_frag_del(_parent, _frag) net_buf_frag_del_debug(_parent, _frag, __func__, __LINE__)
#else
