...
#define Z_MEM_SLAB_INITIALIZER(obj, slab_buffer, slab_block_size, slab_num_blocks)                 \
    {                                                                                              \
        .num_blocks = slab_num_blocks, .block_size = slab_block_size, .buffer = slab_buffer,       \
        .free_list = NULL, .num_used = 0,                                                          \
    }
...
#define K_MEM_SLAB_DEFINE(name, slab_block_size, slab_num_blocks, slab_align)                      \
    char _k_mem_slab_buf_##name[(slab_num_blocks)*WB_UP(slab_block_size)];                         \
    struct k_mem_slab name = Z_MEM_SLAB_INITIALIZER(name, _k_mem_slab_buf_##name,                  \
                                                    WB_UP(slab_block_size), slab_num_blocks)

#define K_MEM_SLAB_NORETENTION_DEFINE(name, slab_block_size, slab_num_blocks, slab_align)          \
    char _k_mem_slab_buf_##name[(slab_num_blocks)*WB_UP(                                           \
            slab_block_size)] __noretention_data_section;                                          \
    struct k_mem_slab name __noretention_data_section = Z_MEM_SLAB_INITIALIZER(                    \
 ...
 *
 */
extern int k_mem_slab_init(struct k_mem_slab *slab, void *buffer, size_t block_size,
                           uint32_t num_blocks);
...
#endif /* _ZEPHYR_POLLING_UTILS_MEM_SLAB_H_ */