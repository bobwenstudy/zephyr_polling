...
help:
	...
	@$(ECHO) "APP:         Select APP Demo built in SDK, will select <example/beacon> by default"
	@$(ECHO) "PORT:        Select Porting info built in SDK, will select <porting/windows_libusb_win32> by default"
	@$(ECHO) "CHIPSET:     Select Chipset built in SDK, will select <chipset/csr8510_usb> by default"
	...
	@$(ECHO) "all [APP=example/beacon] [PORT=porting/windows_libusb_win32] [CHIPSET=chipset/csr8510_usb]"
	@$(ECHO) "   Build a software program."
	@$(ECHO) "2. Show menuconfig:"
	@$(ECHO) "menuconfig [APP=example/beacon] [PORT=porting/windows_libusb_win32] [CHIPSET=chipset/csr8510_usb]"
	@$(ECHO) "   Use menuconfig to show the Kconfig setting."
	@$(ECHO) "3. Show guiconfig:"
	@$(ECHO) "guiconfig [APP=example/beacon] [PORT=porting/windows_libusb_win32] [CHIPSET=chipset/csr8510_usb]"
